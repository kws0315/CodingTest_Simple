...
def printNodes(start):
    ...
    if current == None:  # 노드가 None일 때 함수에서 나온다.
        return
...
def makeList(namePhone):
    ...
    # 리스트가 비어있을 때, head가 방금 생성한 node를 바라보게한다.
    if head == None:
        head = node
        return
    ...
    if head.data[1] > namePhone[1]:
        # 이번엔 이름이 아니라 height를 기준으로 하기 떄문에 [1]로 설정해야한다
        node.link = head
        head = node
        return
    ...
    while current.link != None:
        ...
        if current.data[1] > namePhone[1]: # node를 삽입할 위치를 찾은 경우
            ...
            node.link = current # 새로운 노드가 현재 노드에 link를 연결해서 중간에 삽입시킨다.
            return
...
if __name__ == '__main__':
    # 솔직히 코드 2개만 변경하면 되는 코드라서 복사해서 과제1번 복사해서 썼습니다.
    for data in dataArr:
        makeList(data)
    printNodes(head) # 이름순으로 정렬해서 전화번호는 순서가 안 맞음